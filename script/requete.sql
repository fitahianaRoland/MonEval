...
WITH classement AS (
    SELECT
        ...
        DENSE_RANK() OVER(PARTITION BY t.id_etape ORDER BY t.heure_arrive - t.heure_depart) AS rang
    FROM
        temps_coureur t
    JOIN
...
select * from classement_general_view where id_equipe = 'EQP001'  ;
...
WITH classement AS (
    SELECT
        ...
        t.heure_arrive,
        t.heure_arrive - t.heure_depart AS duree,
        DENSE_RANK() OVER(PARTITION BY cc.id_categorie ORDER BY t.heure_arrive - t.heure_depart) AS rang
    FROM
        temps_coureur t
    JOIN
        coureur c ON t.id_coureur = c.id
...
WITH classement AS (
    SELECT
        ...
        t.heure_arrive,
        t.heure_arrive - t.heure_depart AS duree,
        DENSE_RANK() OVER(PARTITION BY t.id_etape,cc.id_categorie ORDER BY t.heure_arrive - t.heure_depart) AS rang
    FROM 
        temps_coureur t
    JOIN
        coureur c ON t.id_coureur = c.id
...
select id_etape,id_equipe,id_coureur,duree from v_etape_coureur_equipe_duree where id_equipe = 'EQP001'and id_etape = 'ETP001'
group by id_etape,id_equipe,id_coureur,duree;

select * from etape_coureur where id_etape = 'ETP001' ;

select * from coureur where id_equipe = 'EQ001' and id not in (select id_coureur from etape_coureur where id_etape = 'ETP001');

CREATE SEQUENCE coefficient_etape_seq;
CREATE TABLE coefficient_etape(
    id VARCHAR PRIMARY KEY DEFAULT CONCAT('COE', LPAD(nextval('coefficient_etape_seq')::TEXT, 3, '0')), 
...
CREATE OR REPLACE VIEW classement_general_coefficient_etape AS
    SELECT 
    c.*,
    COALESCE(ce.coefficient, 1) AS coefficient
    FROM classement_general_view c
    LEFT JOIN coefficient_etape ce ON c.id_etape = ce.id_etape ;


CREATE OR REPLACE VIEW classement_general_coefficient_etape_rang AS
    SELECT 
    cc.*,
    coefficient * point as point_coefficient,
    DENSE_RANK() OVER(PARTITION BY cc.id_etape ORDER BY (point * coefficient)) AS rang_coefficient
    FROM classement_general_coefficient_etape as cc;

select id_etape , etape , sum(point) , coefficient point from classement_general_coefficient_etape 
group by id_etape , etape , coefficient;

insert into coefficient_etape(id_etape,coefficient) values('ETP001',2) ;